...
def shootlaser(duration_s: float = 2.0) -> None:
    """Fire the laser for duration_s seconds."""
...
def runturrets(theID):
  ...
  for t in rotated_Turrets:
      ...
      y = t.rval * math.sin(t.theta_rad) + globe_r_example[1]
      z = t.zval
...
<body>
    ...
    <fieldset>
        ...
        <form method="POST" action="/laser">
            <button type="submit">Shoot Laser</button>
        </form>
    </fieldset>
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/laser":
            def fire():
                print("[SERVER] Shooting laser")
                shootlaser()
                print("[SERVER] Laser done")
